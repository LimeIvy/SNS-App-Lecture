...
#### Supabase テーブル作成 (`replies`)

- **目的:** 投稿への返信コメントを格納する。
- **テーブル名:** `replies` (当初 `comments` の予定だったが変更)
- **実行方法:** Supabase SQL Editor を使用。
- **スキーマ:**
...
#### 返信関連の型定義追加 (`types/index.ts`)

- **目的:** 返信データおよび返信者プロフィールを含む返信データの型を定義する。
- **ファイル:** `types/index.ts`
- **内容:**
  - `Reply` インターフェース (id, user_id, post_id, content, created_at) を定義。
  - `ReplyWithProfile` インターフェース (`Reply` を拡張し `profile: Profile | null` を追加) を定義。

#### 返信作成 API (`app/api/posts/[postId]/reply/route.ts`)

- **目的:** 特定の投稿に対して認証済みユーザーが返信を作成できるようにする。
- **ファイル:** `app/api/posts/[postId]/reply/route.ts` (新規作成)
- **エンドポイント:** `POST /api/posts/:postId/reply`
- **処理内容:**
  - リクエストボディから `content` (返信内容) を取得し、空でないことをバリデーション。
  - Supabase サーバーサイドクライアントを使用して、認証済みユーザーの ID (`user.id`)、リクエストパスから取得した投稿 ID (`params.postId`)、および `content` を `replies` テーブルに挿入。
  - 挿入後、新しく作成された返信データを、返信者のプロフィール情報 (`name`, `icon`) と共に取得し直す (結合クエリを使用)。
  - 成功時 (HTTP 201) は、取得した返信データ (プロフィール情報込み) を JSON で返す。
  - ユーザー未認証の場合は 401 エラー、`content` が空の場合は 400 エラー、その他のエラー (DBエラーなど) の場合は 500 エラーを返す。

#### 返信入力フォーム (`components/ReplyForm.tsx`)

- **目的:** ユーザーが返信を入力し送信できる UI コンポーネント。
- **ファイル:** `components/ReplyForm.tsx` (新規作成)
- **Props:** `postId` (返信対象の投稿ID), `onReplySuccess` (返信成功時に呼び出されるコールバック関数)。
- **内容:**
  - クライアントコンポーネント (`"use client"`)。
  - `useState` で入力内容 (`content`)、ローディング状態 (`isLoading`)、エラーメッセージ (`error`) を管理。
  - `useEffect` でログインユーザーのアイコンを取得し表示。
  - `handleSubmit` 関数:
    - `content` が空の場合はエラーメッセージを設定して処理を中断。
    - `isLoading` を `true` に設定。
    - `fetch` API を使用して `/api/posts/${postId}/reply` (POST) に `content` を送信。
    - レスポンスが成功 (201 Created) の場合:
      - `onReplySuccess` コールバックを呼び出し、レスポンスボディ (新しい返信データ) を渡す。
      - `content` をクリア。
    - エラーが発生した場合は `error` State にメッセージを設定。
    - `finally` ブロックで `isLoading` を `false` に戻す。
  - UI はテキストエリア、送信ボタン、ローディング/エラー表示で構成。

#### 返信一覧取得 API (`app/api/posts/[postId]/replies/route.ts`)

- **目的:** 特定の投稿に紐づく返信の一覧を、返信者のプロフィール情報と共に取得する。
- **ファイル:** `app/api/posts/[postId]/replies/route.ts` (新規作成)
- **エンドポイント:** `GET /api/posts/:postId/replies`
- **処理内容:**
  - リクエストパスから投稿 ID (`params.postId`) を取得。
  - Supabase サーバーサイドクライアントを使用して、`replies` テーブルから `post_id` が一致するデータを取得。
  - 取得時に `profile` テーブルを結合し、各返信に投稿者の `id`, `name`, `icon` を含める。
  - `created_at` カラムで降順 (新しい順) にソート。
  - 成功時は取得した返信データ (または空配列) を JSON で返す。
  - エラー発生時は 500 エラーを返す。

#### 投稿詳細ページへの返信一覧表示 (`app/[userId]/[postId]/page.tsx`)

- **目的:** 投稿詳細ページに、上記 API を使用して取得した返信の一覧を表示する。
- **変更箇所:** `app/[userId]/[postId]/page.tsx`
- **内容:**
  - **State 変数追加:**
  ...
  - **`useEffect` フックの拡張:**
    - 既存の投稿データ取得処理に加えて、返信一覧取得処理を追加。
    - `fetch` API を使用して `/api/posts/${postId}/replies` (GET) を呼び出し、レスポンスを `replies` State にセット。
    - ローディング状態 (`isLoadingReplies`) とエラー状態 (`repliesError`) を管理。
  - **返信成功時の処理 (`handleReplySuccess` 関数):**
    - `ReplyForm` の `onReplySuccess` プロップに渡すコールバック関数。
    - 新しい返信オブジェクト (`newReply: ReplyWithProfile`) を引数に取り、`replies` State の先頭に追加することで、UIに即時反映させる。
    - (Linterエラー対応のため、`newReply` の型を `unknown` とし、内部で `ReplyWithProfile` に型アサーションするよう修正)
  - **JSX での表示:**
    - `isLoadingReplies`, `repliesError`, `replies.length` の状態に応じて、ローディング表示、エラーメッセージ、または「まだ返信はありません。」というメッセージを表示。
    - `replies` 配列を `map` して各返信アイテムを表示。
      - 各返信アイテムには、返信者のアイコン、名前 (`reply.profile.name`)、ユーザーID (`reply.profile.id`)、返信日時 (`reply.created_at` をフォーマット)、返信内容 (`reply.content`) を表示。
      - 各返信アイテムに、リプライ、リツイート、いいねのアイコンボタン（機能は未実装、見た目のみ）を追加。
- **確認方法:**
  - 投稿詳細ページを開くと、ローディング表示の後、その投稿への返信が新しい順に表示されることを確認。
  - 各返信には、投稿者のアイコン・名前・ID・返信日時・内容、およびアクションボタンが表示されることを確認。
  - 返信フォームから新しい返信を投稿すると、API経由で保存され、ページをリロードせずに即座に返信一覧の先頭に追加表示されることを確認。
  - 返信がない場合は「まだ返信はありません。」と表示されることを確認。
  - 返信取得に失敗した場合はエラーメッセージが表示されることを確認。
